...
async function generateExamples(
  ...
  const system =
    `Generate exactly ${n} SHORT, natural English example sentences (6–12 words, ` +
    `easy A2–B1 level) that use the word "${word}" naturally and clearly show its ` +
    `meaning. Keep them everyday and appropriate for all ages. Give a Brazilian ` +
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (body.mode === 'examples') {
      ...
      let examples = await tatoeba(q, limit)
      ...
      if (examples.length < limit) {
        const generated = await generateExamples(q, limit - examples.length)
